...
@Service(value="DotacaoService")
public class DotacaoService extends FormulariosTREDFService {
  ...
  @Autowired
  private JmsTemplate jmsSaoArquivoDotacaoTemplate;
  
  @Autowired
  private DotacaoDao dotacaoDao;
  
  public void enviaArquivoDotacao(final ArquivoEnviadoSao arquivo, String token) {
  	if(arquivo == null || StringUtils.isEmpty(arquivo.getDataReferencia()) ) {
  		String msg = MessageFormat.format(bundle.getString("DotacaoService.enviaarquivo.erro")
    ...
    List<Permissao> permissoes = addNovaPermissaoList(null, 
        properties.getProperty("perm.upload.arquivo.dotacao.sao"));    
    validaTokenUsuario(token, permissoes);
  	
    jmsSaoArquivoDotacaoTemplate.send(new MessageCreator() {			
			@Override
  ...
  public List<FormularioDotacao> getFormulariosDotacao(String token) {
    List<Permissao> permissoes = addNovaPermissaoList(null, 
        properties.getProperty("perm.upload.arquivo.dotacao.sao"));    
    validaTokenUsuario(token, permissoes);
  	return dotacaoDao.getFormulariosDotacao();
  }
  
  public FormularioDotacao getFormularioDotacaoPorId(String token, Long id) {
    List<Permissao> permissoes = addNovaPermissaoList(null, 
        properties.getProperty("perm.upload.arquivo.dotacao.sao"));    
    validaTokenUsuario(token, permissoes);
  	return dotacaoDao.getFormularioDotacaoPorId(id);
  }
  
  public FormularioDotacao getFormularioDotacaoPorDtReferencia(String token, String dtReferencia) {
    List<Permissao> permissoes = addNovaPermissaoList(null, 
        properties.getProperty("perm.upload.arquivo.dotacao.sao"));    
    validaTokenUsuario(token, permissoes);
  	return dotacaoDao.getFormularioDotacaoPorDtReferencia(FormularioExecucao.getDtReferenciaFormatada(dtReferencia));
  }
  
  public void apagarFormularioDotacao(String token, Long id) {
    List<Permissao> permissoes = addNovaPermissaoList(null, 
        properties.getProperty("perm.upload.arquivo.dotacao.sao"));    
    validaTokenUsuario(token, permissoes);
    dotacaoDao.apagarFormularioDotacao(id);
    UsuarioVO usuario = authenticationService.getUsuarioFromToken(token);
		...
		log.setDescricao(bundle.getString("Execucao.removendo.arquivo.dotacao"));
